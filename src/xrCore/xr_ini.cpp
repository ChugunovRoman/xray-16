...
XRCORE_API CInifile const* pSettingsFE = nullptr;
...
CInifile::Sect& CInifile::r_section(pcstr S) const
{
    ...
    else if (xr_strcmp((*I)->Name.c_str(), section))
    {
        ...
        // FS.w_close (F);
        if (strstr(Core.Params, "-skpsct") != NULL)
            Msg("! ERROR: Can't open section '%s' (only '%s' avail). Please attach [*.ini_log] file to your bug report", section, (*I)->Name.c_str());
        else
            xrDebug::Fatal(DEBUG_INFO, "! ERROR: Can't open section '%s' (only '%s' avail). Please attach [*.ini_log] file to your bug report", section, (*I)->Name.c_str());
    }
    return **I;
}
