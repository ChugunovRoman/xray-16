...
void attachable_hud_item::render(u32 context_id, IRenderable* root)
{
    ...
    if (wpn && wpn->bUseAttachmentSystem)
    {
        for (auto slot: wpn->m_addon_slots)
            render.draw_aabb(slot.second.c, 0.003f, 0.003f, 0.003f, color_xrgb(0, 255, 0));
...
void attachable_hud_item::calc_addon_aim_offset()
{
    ...
    for (auto [addon_id, item] : wpn->m_addon_items)
    {
        ...
        if (bone_id == BI_NONE || item->parent_id != 0 || item->slot != "slot_1") continue;

        m_measures.m_hands_offset[0][1].set(item->calc_aim_offset);
        m_measures.m_hands_offset[1][1].set(item->calc_aim_rot);
...
void player_hud::calc_transform(u16 attach_slot_idx, const Fmatrix& offset, const Fmatrix& offset2, Fmatrix& result, Fmatrix& result2, Fmatrix& result3) const
{
    ...
    if (item && !item->m_monolithic)
    {
        ...
        Msg("player_hud::calc_transform, m_sect_name: %s bone_id: %d, bone_transform: %f %f %f rot: %f %f %f", item->m_sect_name.c_str(), bone_id, bone_transform.c.x, bone_transform.c.y, bone_transform.c.z, rot.x, rot.y, rot.z);
