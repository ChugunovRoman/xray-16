...
void CUIActorMenu::QuickUnloadWeapons()
{
    ...
    for (PIItem const item : ruck_list)
    {
        ...
        if (m_wpn->bUseAttachmentSystem && !m_wpn->m_addon_items.empty())
        {
            xr_vector<u32> addons_to_detach;
            addons_to_detach.reserve(m_wpn->m_addon_items.size());
            for (auto& [addon_id, addon] : m_wpn->m_addon_items)
            {
                if (m_wpn->IsAddonCanBeDetached(addon))
                    addons_to_detach.push_back(addon_id);
            }
            for (u32 addon_id : addons_to_detach)
            {
                if (m_wpn->m_addon_items.find(addon_id) != m_wpn->m_addon_items.end())
...
void CUIActorMenu::highlight_item_slot(CUICellItem* cell_item)
{
    ...
    if (bolt && slot_id == BOLT_SLOT)
    {
        if (m_pLists[eInventoryBoltList])
            m_pLists[eInventoryBoltList]->Highlight(true);
        return;
    }
...
void CUIActorMenu::highlight_outfits_for_patch(PIItem patch_item, CUIDragDropListEx* ddlist)
{
    ...
    pcstr item_class = READ_IF_EXISTS(pSettings, r_string, patch_item->m_section_id.c_str(), "item_class", nullptr);
