...
bool CUIActorMenu::OnItemDropped(PIItem itm, CUIDragDropListEx* new_owner, CUIDragDropListEx* old_owner)
{
    ...
    // TODO: FIX add support slot and ort
    AttachAddon(_iitem, CurrentIItem());
...
void CUIActorMenu::PropertiesBoxForAddon(PIItem item, bool& b_show)
{
    ...
    if (pScope)
    {
        if (item_in_slot_2 && wpn_2 && item_in_slot_2->CanAttach(pScope))
        {
            ...
            {
                auto slots = wpn_2->getAvaliableSlots();
                for (auto slot : slots)
                {
                    ...
                    if (slot.parent == 0)
                        str.printf("%s %s %s%s", str.c_str(), item_in_slot_2->m_name.c_str(), *slot.slot_name, ort != nullptr ? *ort : "");
                    else
                    {
                        shared_str addon_name = StringTable().translate(pSettings->r_string(slot.parent_section, "inv_name"));
                        str.printf("%s %s %s %s%s", str.c_str(), item_in_slot_2->m_name.c_str(), *addon_name, *slot.slot_name, ort != nullptr ? *ort : "");
                    }
        ...
        if (item_in_slot_3 && wpn_3 && item_in_slot_3->CanAttach(pScope))
        {
            ...
            {
                auto slots = wpn_3->getAvaliableSlots();
                for (auto slot : slots)
                {
                    if (pScope->HasScopeTexture() && (slot.parent != 0 || slot.slot_name != "slot_1"))
                        continue;
                    ...
                    shared_str str = StringTable().translate("st_attach_scope_to_pistol");
                    if (slot.parent == 0)
                        str.printf("%s %s %s%s", str.c_str(), item_in_slot_3->m_name.c_str(), *slot.slot_name, ort != nullptr ? *ort : "");
                    else
                    {
                        shared_str addon_name = StringTable().translate(pSettings->r_string(slot.parent_section, "inv_name"));
                        str.printf("%s %s %s %s%s", str.c_str(), item_in_slot_3->m_name.c_str(), *addon_name, *slot.slot_name, ort != nullptr ? *ort : "");
                    }
                    if (slot.busy_by != nullptr)
                    {
                        shared_str addon_name = StringTable().translate(pSettings->r_string(*slot.busy_by, "inv_name"));
                    ...
                    Msg("attach addon (PropertiesBoxForAddon): %s slot.slot_name: %s slot.parent: %d pScope->m_has_ort: %d str: [%s]", *item_in_slot_3->m_section_id, slot.slot_name.c_str(), slot.parent, pScope->m_has_ort, str.c_str());
                    AttchItemData* item = xr_new<AttchItemData>();
                    ...
                    if (pScope->m_has_ort)
                    {
                        ort = StringTable().translate("st_attach_ort_r");
                        str = StringTable().translate("st_attach_scope_to_pistol");
                        Msg("attach addon (PropertiesBoxForAddon): %s pIItem->attach_to_slot_name: %s", *item_in_slot_3->m_section_id, item_in_slot_3->attach_to_slot_name.c_str());
                        if (slot.parent == 0)
                            str.printf("%s %s %s%s", str.c_str(), item_in_slot_3->m_name.c_str(), *slot.slot_name, ort != nullptr ? *ort : "");
                        else
                        {
...
void CUIActorMenu::ProcessPropertiesBoxClicked(CUIWindow* w, void* d)
{
    ...
    case INVENTORY_ATTACH_ADDON:
    {
        ...
        Msg("INVENTORY_ATTACH_ADDON, item [%s] attach_to_slot_name: %s, parent_addon: %d, attach_to_ort: %d", item->m_section_id.c_str(), *item->attach_to_slot_name, item->parent_addon, item->attach_to_ort);
